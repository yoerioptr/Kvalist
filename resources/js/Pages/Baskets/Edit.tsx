...
export default function Edit({ basket, stores, products, units }: Props) {
    const { data, setData, patch, processing, errors } = useForm({
        name: basket.name,
        store_id: basket.store_id || '',
        new_store_name: '',
        items: basket.items.map((item, index) => ({
            id: crypto.randomUUID(), // Always give a fresh ID for DnD consistency
    ...
    const submit: FormEventHandler = (e) => {
        ...
        patch(route('baskets.update', { basket: basket.id }));
    };
    ...
    return (
        <AuthenticatedLayout
            header={
                <Heading>Edit Basket</Heading>
            }
        >
            <Head title="Edit Basket" />
